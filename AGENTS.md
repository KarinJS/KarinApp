...
- 底部导航 Home / Plugins / Settings；明暗主题跟随系统。`BottomNav` 是悬浮玻璃底栏（64dp 控件 + 上下各 8dp + 系统底部 inset），`useBottomNavInset` 统一提供页面避让高度；三个主页面滚动内容延伸至底栏背后，日志 / 文件 / 设置子页保留底部空间。背景模糊由 `KarinGlassView` 只采样 `nativeID="karin-tab-scene"` 的独立页面视图（必须 `collapsable={false}`，不能把导航放进采样源），Android 12+ 用 RenderEffect、7–11 用 RenderScript，失败保留半透明底色。系统导航栏透明，`MainActivity` 同步手势条明暗色；不要恢复根 SafeAreaView 的 bottom padding 导致重复留白。底栏支持单指横向拖动，选中胶囊跟手并预览目标，松手吸附后才切页（途经页面不挂载）；拖出底栏、多指或系统打断取消，拖回当前 Tab 不触发 `onSelect`，保留日志/文件子页。普通点按与无障碍操作保留，切换动画尊重系统减少动画设置。
